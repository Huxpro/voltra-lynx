...
export function FlexPlaygroundScreen() {
  ...
  const cycleFlexDirection = useCallback(() => {
    setFlexDirection((prev: FlexDirection) => {
      const currentIndex = FLEX_DIRECTION_OPTIONS.indexOf(prev);
      return FLEX_DIRECTION_OPTIONS[(currentIndex + 1) % FLEX_DIRECTION_OPTIONS.length];
    });
  }, []);

  const cycleAlignItems = useCallback(() => {
    setAlignItems((prev: AlignItems) => {
      const currentIndex = ALIGN_ITEMS_OPTIONS.indexOf(prev);
      return ALIGN_ITEMS_OPTIONS[(currentIndex + 1) % ALIGN_ITEMS_OPTIONS.length];
    });
  }, []);

  const cycleJustifyContent = useCallback(() => {
    setJustifyContent((prev: JustifyContent) => {
      const currentIndex = JUSTIFY_CONTENT_OPTIONS.indexOf(prev);
      return JUSTIFY_CONTENT_OPTIONS[(currentIndex + 1) % JUSTIFY_CONTENT_OPTIONS.length];
    });
  }, []);

  const increaseGap = useCallback(() => setGap((prev: number) => Math.min(prev + 4, 32)), []);
  const decreaseGap = useCallback(() => setGap((prev: number) => Math.max(prev - 4, 0)), []);

  const increasePadding = useCallback(() => setContainerPadding((prev: number) => Math.min(prev + 4, 32)), []);
  const decreasePadding = useCallback(() => setContainerPadding((prev: number) => Math.max(prev - 4, 0)), []);
    ...
    <scroll-view style={{ flex: 1 } as any} scroll-orientation="vertical">
      <view style={{ paddingLeft: 20, paddingRight: 20, paddingTop: 24, paddingBottom: 24 }}>
        ...
        <view style={{ backgroundColor: '#1E293B', borderRadius: 12, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#334155' } as any}>
          ...
          {/* Gap */}
          <view style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <text style={{ fontSize: 14, color: '#CBD5F5' } as any}>Gap: {gap}px</text>
          ...
          <view style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <text style={{ fontSize: 14, color: '#CBD5F5' } as any}>Padding: {containerPadding}px</text>
            <view style={{ flexDirection: 'row', gap: 8 }}>
              <view
        ...
        <view style={{ backgroundColor: '#1E293B', borderRadius: 12, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#334155' } as any}>
          ...
          <view style={{
            ...
            <view style={{
              ...
              <text style={{ color: '#FFFFFF', fontSize: 12, fontWeight: 'bold' } as any}>Item 1</text>
            </view>
            <view style={{
              ...
              <text style={{ color: '#FFFFFF', fontSize: 12, fontWeight: 'bold' } as any}>Item 2</text>
            </view>
            <view style={{
              ...
              <text style={{ color: '#FFFFFF', fontSize: 12, fontWeight: 'bold' } as any}>Item 3</text>
            </view>
          </view>
        </view>
        ...
        <view style={{ backgroundColor: '#1E293B', borderRadius: 12, padding: 16, marginTop: 16, borderWidth: 1, borderColor: '#334155' } as any}>
          ...
          <view style={{
            ...
            <view style={{ backgroundColor: '#1E293B', padding: 8, flex: 1 }}>
              <text style={{ color: '#FFFFFF', fontSize: 14, textAlign: 'left' } as any}>textAlign: left</text>
            </view>
            <view style={{ backgroundColor: '#1E293B', padding: 8, flex: 1 }}>
              <text style={{ color: '#FFFFFF', fontSize: 14, textAlign: 'center' } as any}>textAlign: center</text>
            </view>
            <view style={{ backgroundColor: '#1E293B', padding: 8, flex: 1 }}>
              <text style={{ color: '#FFFFFF', fontSize: 14, textAlign: 'right' } as any}>textAlign: right</text>
            </view>
          </view>
        </view>
      </view>
