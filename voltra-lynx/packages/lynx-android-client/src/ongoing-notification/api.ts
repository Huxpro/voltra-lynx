...
import { isAndroid } from '@voltra-lynx/bridge'
import { useUpdateOnHMR } from '../utils/index.js'
import VoltraModule from '../VoltraModule.js'

const isAndroidNotificationPermissionRequired = (): boolean => {
  // In Lynx, check via native module capabilities
  const caps = VoltraModule.getAndroidOngoingNotificationCapabilities()
  return isAndroid() && caps.apiLevel >= 33
}
...
export const hasAndroidNotificationPermission = async (): Promise<boolean> => {
  ...
  // In Lynx, notification permission check uses native module
  const caps = VoltraModule.getAndroidOngoingNotificationCapabilities()
  return caps.notificationsEnabled
}
...
export const requestAndroidNotificationPermission = async (): Promise<boolean> => {
  ...
  // In Lynx, permission request would be handled via native module
  // For now, check current state via capabilities
  const caps = VoltraModule.getAndroidOngoingNotificationCapabilities()
  return caps.notificationsEnabled
}
