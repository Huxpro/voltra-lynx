...
enum PreloadError: Error, LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case let .httpError(statusCode): return "HTTP error: \(statusCode)"
    case let .imageTooLarge(key, size): return "Image '\(key)' is too large: \(size) bytes (max \(VoltraImagePreload.maxImageSizeInBytes) bytes)"
    case let .invalidImageData(key): return "Invalid image data for '\(key)'"
    case .appGroupNotConfigured: return "App Group not configured"
    }
  }
}
